...
- 只执行 `anna` 单人 slow 路线；所有日更入口默认进入 Kie 确认图流程，不存在其他视频模式或自动切换。
...
- Dreamina 视频只允许上传用户确认的 Kie 原始图；`anna.png` 只用于 Kie 确认图。参考宫格只用于分析。
...
- 环境修复和最佳实践沉淀只看模块 00；行程、参考和 prompt 规范只看模块 01；确认图只看模块 02；视频输入与确认节点只看模块 03；发布只看模块 04。
