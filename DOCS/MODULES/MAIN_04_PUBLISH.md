...
- 抖音封面固定使用视频中间帧。
- 抖音将去重后的前 4 个项目候选话题以 `#话题` 形式直接写入作品描述，不要求匹配或选择平台建议。点击发布前回读作品描述；实际 `#话题` 不是恰好 4 个、缺少计划话题或存在计划外话题时阻断。报告分别保存 `requested_tags` 与 `applied_tags`。
- 两个平台实际应用标签均固定为 4 个。快手可接收更大的候选池，但只把去重后的前 4 个写入文案；不足 4 个时阻断，并分别记录 requested 与 applied。
