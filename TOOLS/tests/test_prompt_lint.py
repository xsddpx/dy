...
GOOD_PROMPT = (
    ...
    "@图2 是参考宫格图，仅用于场景、分镜、镜头视角、穿搭版型、动作节奏和封面停顿参考。"
...
class PromptLintFlowTest(unittest.TestCase):
    ...
    def test_without_confirmation_image_fails(self):
        result = self.lint("@图2 是参考宫格图，室内镜前自然移动。")
        self.assertEqual(result["decision"], "fail")
        self.assertTrue(any(f["code"] == "missing_confirmation_image" for f in result["findings"]), result["findings"])

    def test_without_reference_grid_fails(self):
        result = self.lint("画面人物以本次确认图为身份锚点，站在室内镜前自然移动。修身剪裁呈现饱满的立体廓形，光影让腰胯比例明显。")
        self.assertEqual(result["decision"], "fail")
        self.assertTrue(any(f["code"] == "missing_reference_grid" for f in result["findings"]), result["findings"])
    ...
    def test_unsupported_terms_fail(self):
        result = self.lint(GOOD_PROMPT + " 模型参数。")
        self.assertEqual(result["decision"], "fail")
        self.assertTrue(any(f["code"] == "unsupported_terms" for f in result["findings"]), result["findings"])

    def test_at_image_two_without_grid_meaning_fails(self):
        result = self.lint(
            "画面人物以本次确认图为身份锚点，@图2 参考姿态。"
            "修身剪裁呈现饱满的立体廓形，光影让腰胯比例明显。"
        )
        self.assertEqual(result["decision"], "fail")
        self.assertTrue(any(f["code"] == "missing_reference_grid" for f in result["findings"]), result["findings"])
    ...
    def test_missing_artistic_body_translation_fails(self):
        result = self.lint("画面人物以本次确认图为身份锚点，@图2 是参考宫格图，站在室内镜前自然移动。")
        self.assertEqual(result["decision"], "fail")
