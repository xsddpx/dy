...
- 衣柜图成对实验在同一 RUN_ID 记录图片衣柜编号与映射、三张输入路径和 SHA-256，以及每个 `vN` 的基线/三图 prompt、各自任务 ID、TNS/终态和 TEMP 视频路径；若同步重试到 `v5`，必须保留全部 2–10 个任务的版本链，不能只记录最终一对。成片另记录人物、服装、环境、动作、人台泄漏五项人工结论。实验不写 `OUTPUT/`、不上传 Drive、不进入发布，相关状态统一记录为 `not_applicable_experiment`。
