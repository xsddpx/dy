...
img prompt 用于确认图阶段，由模块 01 的十段式 `grid-prompt.txt` 删除 `整体动画：` 和 `背景音乐：` 两段得到：
...
- `人物：`、`视频类型：`、`穿搭：`、`姿态镜头：`、`环境：`、`卖点与锁定：`、`表情节奏：`、`其他：` 均保留自同一份 `grid-prompt.txt`，只做删除动画和音乐两段的阶段裁剪。
- 每段冒号后都必须保留最终可执行内容，不得保留省略号、尖括号占位、条件分支或模板说明。
- 最终 img prompt 不得写旧参考类型字段、文件名、来源说明、流程说明、合规说明、平台解释、第二张图片引用或“根据/融合/吸收”等解释性表达。
- 若删除动画和音乐后出现断裂、矛盾、动态动作过强，或穿搭、姿态镜头、环境、卖点与锁定不够具体，必须回到模块 01 重写 `grid-prompt.txt`，再派生 img prompt。

## 生成前自检

提交 Kie 前，执行者必须检查最终 img prompt：

- 已使用从十段式 `grid-prompt.txt` 派生出的八段结构，并包含 `视频类型：`、`卖点与锁定：` 和 `表情节奏：`。
...
- 最终文本不含内部分析标签、文件名、来源说明、合规说明、平台解释、第二张图片引用或不可发布内容。
...
- img prompt 使用 `人物：`、`视频类型：`、`穿搭：`、`姿态镜头：`、`环境：`、`卖点与锁定：`、`表情节奏：`、`其他：` 八段结构。
