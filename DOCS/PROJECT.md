...
以下是基于 2026-06-26 创作者中心可见作品表现形成的阶段性策略，不是永久规则；每累计 10 条新发布作品，或账号内容方向明显变化时，按模块 06 重新复盘并更新。
...
- `DOCS/MODULES/MODULE_06_OPERATION_REVIEW.md`：运营复盘和下一条选题决策规则；只在用户要求复盘、累计足够新样本或需要更新阶段性策略时执行，不是发布硬节点。
...
8. 记录收尾：成功生成正式视频后写入去重账本；发布后只在运行记录中补充发布状态并刷新记录；运营复盘按模块 06 独立执行，不把本次运行状态作为下次默认续跑依据。
