...
- 确认临时 MP4 可提取首中尾三帧。大小、哈希、播放、抽帧或解码出现异常迹象时，再按模块 00 复用原 `submit_id` 取回并严格解码，不改变任务和 prompt。
...
- 两项硬门通过后，将临时 MP4 整理为 `OUTPUT/RUN_ID.mp4`；运行记录写入固定角色代理图、三张视频代理图和人工结论，发布与归档始终使用原始 MP4。

## 发布路由

- `OUTPUT/RUN_ID.mp4` 完成质检后默认进入模块 03。
- 用户明确要求“发布前确认”“只生成不发布”或“本次不用发布”时在此硬停，展示正式视频、首中尾帧、vid prompt、TNS 记录和发布建议；取得明确授权后再进入模块 03。

## TNS 重试

- `vid-prompt-v1.txt` 是首次提交。仅当 Dreamina 明确返回 TNS/安全拦截且没有可下载 MP4 时，才回模块 01 在固定合同内重选衣柜款式或动画模板，生成 `vid-prompt-v2.txt` 至 `vid-prompt-v5.txt`。
- 每版重新运行 prompt lint，通过后再提交；固定角色图、单个 `@图1` 和环境模板保持不变，视频类型与所选动画模板保持对应。
