...
本模块自身不触发 xdy、xdysp、Dreamina 视频生成、Google Drive 或平台发布。模块 01/02 的衣柜图实验模式可以读取本模块已入库图片，但默认双图日更仍不消费这些资产；实验是否升级为正式主链由成对验证结果另行决定。
...
- 画幅固定为 9:16 竖版正面全身图；人台居中、四周留白均匀，服装主体尽量充满安全画幅，所有入库图片使用一致的构图和视觉尺度。
...
正式图片目录仍然只保留 PNG，但每次成功入库必须在 `MATERIAL/anna-wardrobe.md` 的“图片衣柜实验款”同步追加同编号映射：

```text
图-NNN.
- 图片路径：MATERIAL/wardrobe-images/衣柜图-NNN.png
- 款式提示词：<从原始参考图可见事实整理出的安全、完整服装描述>
- 视频状态：experimental
```

图片编号、映射编号和路径编号必须一致。款式提示词只记录服装可见事实，并遵守模块 01 的 TNS 词汇约束；新条目初始状态固定为 `experimental`，不能因为静态商品图质检通过而直接写成 `validated`。映射写入和 PNG 正式命名视为同一次入库：任一部分失败都不得报告入库成功，也不得留下无映射的正式图片。
...
正式日更主链及其分支以 `DOCS/PROJECT.md` 为唯一事实源。新入库的人台商品图不会自动改变默认衣柜选择、Dreamina 输入、日更 prompt、视频生成或发布流程；只有用户明确进入衣柜图实验模式时，模块 01 才能通过 `MATERIAL/anna-wardrobe.md` 的映射锁定其中一张图片供模块 02 三图验证。
