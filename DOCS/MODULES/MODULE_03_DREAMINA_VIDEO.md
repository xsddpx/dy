...
- 在默认 `auto/fast` 模式下，根据 `anna.png` 和模块 01 的 `grid-prompt.txt` 生成最终 vid prompt。
- 在显式 `slow` 模式下，根据模块 02 用户确认后的 Kie 原始图和同一份 `grid-prompt.txt` 生成最终 vid prompt。
- 提交 Dreamina `multimodal2video` 并下载正式 MP4。

## prompt 边界

- `grid-prompt.txt` 是 img/vid prompt 的统一规则库；视频阶段从其中取用八段式标准块，并清理为 Dreamina 可直接执行的画面描述。
...
- 最终 vid prompt 不得出现 `grid-prompt.txt`、`reference-grid`、`参考宫格`、`参考类型识别`、`主类型=`、`次类型=`、`判断依据=`、`同时吸收`、`根据 grid`、`根据文档`、`上述分析`、`文件`、`流程`、`节点` 等内部来源词或解释性词。

## vid prompt 格式

最终 vid prompt 必须使用八段式结构：
...
- `人物：` 来自模块 01 的固定人物段。`auto/fast` 以 `anna.png` 作为 `@图1` 的身份、五官、发型、脸型和稳定身材比例依据；穿搭、动作、场景来自本次 `grid-prompt.txt`，不得写“以 @图1 中的穿搭作为依据”或同义表达。`slow` 以选中图作为 `@图1` 的身份、姿态和画面锚点，并与 `grid-prompt.txt` 的标准块保持一致。
- `穿搭：`、`姿态镜头：`、`环境：`、`卖点与锁定：`、`其他：` 从模块 01 的对应标准块取用，并清理为可直接执行文本。
- `整体动画：` 必须以 `视频类型为主类型，次类型为次类型；` 开头；次类型为无时写 `视频类型为主类型，次类型为无；`。vid prompt 必须围绕约 5-6 秒成片重新编排动画，不按参考原时长逐秒照搬。
- `整体动画：` 必须写清连续动作、手部路径、身体重心变化、镜头距离、角度、运镜、节奏点、参考表情节奏和结尾停顿；`auto/fast` 最终表达必须与 `anna.png` 角色身份不冲突，`slow` 最终表达必须与选中图不冲突。
- `背景音乐：` 只写音乐风格、节奏和情绪，不写歌词、对白、口播或任何非音乐声音。
- `其他：` 必须写真实皮肤纹理、自然光影、真实面料质感、构图稳定、画面物理真实，并明确除背景音乐外不出现环境声、人声、脚步声、衣料声、镜头声、口播、对白、喘息或音效。
...
- `TEMP/RUN_ID/grid-prompt.txt` 存在，且已记录九段式结构，能支持 vid 八段式拼装。
- `vid prompt` 已由 `grid-prompt.txt` 的八段式标准块人工整理为可直接执行的 Dreamina 画面描述。
- 最终 prompt 不含第二张图片引用、文件名、流程说明、来源说明或“吸收/根据某文件”的表达。
- 未上传任何参考图、参考帧或参考宫格图。
...
- `TEMP/RUN_ID/grid-prompt.txt` 存在，且已记录九段式结构，能支持 vid 八段式拼装。
- `vid prompt` 已由同一份 `grid-prompt.txt` 的八段式标准块重新整理为可直接执行的 Dreamina 画面描述，并与用户确认的单图不冲突。
- 最终 prompt 已显式写入视频类型并承接参考表情节奏，且不含第二张图片引用、文件名、流程说明或确认图阶段解释。
...
- vid prompt 使用 `人物：`、`穿搭：`、`姿态镜头：`、`环境：`、`卖点与锁定：`、`整体动画：`、`背景音乐：`、`其他：` 八段式。
- `整体动画：` 已以 `视频类型为...，次类型为...；` 开头，类型来自模块 01 的合法集合。
- `整体动画：` 已承接模块 01 的参考表情节奏，表情随动作、镜头距离和场景自然变化，不把微笑作为默认模板。
- vid prompt 不含第二张图片引用，不引入参考图视觉输入语义，不含文件名、流程说明、来源说明、直白身材词或音乐以外的其他声音。
