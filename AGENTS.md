...
- `slow` 的确认图 img prompt 绕过 `grid-prompt.txt`：执行者必须直接查看 `reference-grid.jpg` 或帧图，重点分析穿搭、环境、人物姿态和镜头关系，并写成 `人物：`、`穿搭：`、`姿态镜头：`、`环境：`、`其他：` 五段式最终 img prompt；特殊或不对称穿搭必须在 `穿搭：` 段锁定左右结构、袖长、领口方向、开口位置、裤腿或裙摆连续性和裁切范围。
