...
模块 01 的参考类型只允许转译成可见画面意图写入 img prompt，例如“穿搭展示式正面站姿关键帧”“镜前自拍式手机近景构图”“走路回头式侧身转身瞬间”。确认图表情保持自然，来自模块 01 的参考表情节奏或 `@图1` 的自然神态，不默认生成笑脸。最终 img prompt 不得写 `参考类型=`、`主类型=`、`次类型=`、`grid-prompt.txt`、`参考宫格`、`类型判断依据`、流程说明或文件来源说明。
