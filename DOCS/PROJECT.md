...
3. 参考选择：按当天行程大方向选择参考；用户指定参考优先，其次从抖音收藏中找匹配当天日期、季节、城市片区、白天/晚上方向、旅行状态或活动氛围的参考；收藏没有合适主题时，才从抖音热门中挑选匹配参考。参考地点不必完全等同行程地点，只要能解释为当天行程附近、途中或延展活动；同等可用参考中优先匹配当前阶段性增长方向；进入流程前先做 7 天去重。
