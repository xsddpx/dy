...
class ConfirmationManifestTest(unittest.TestCase):
    def setUp(self):
        ...
        self.image2 = self.root / "image2.png"
        self.image1.write_bytes(PNG_1X1)
        self.image2.write_bytes(PNG_1X1)
    ...
    def test_two_success_slots_are_preserved(self):
        proc = self.run_manifest([
            ...
            {
                "slot": "A-02",
                "submit_id": "22222222-ffff",
                "status": "success",
                "image_path": str(self.image2),
                "model_version": "nano-banana-pro-1K",
            },
        ])
        self.assertEqual(proc.returncode, 0, proc.stdout + proc.stderr)
        manifest = json.loads((self.out_dir / "confirmation-manifest.json").read_text(encoding="utf-8"))
        self.assertEqual([item["slot"] for item in manifest["slots"]], ["A-01", "A-02"])
        self.assertEqual([item["status"] for item in manifest["slots"]], ["success", "success"])
        ...
        self.assertIn("22222222", manifest["slots"][1]["proxy_path"])
    ...
    def test_rejects_missing_slots_instead_of_backfilling(self):
        ...
        self.assertIn("每批必须正好 2 个提交位", proc.stderr)

    def test_rejects_legacy_slots(self):
        proc = self.run_manifest([
            {"slot": "A-01", "submit_id": "1", "status": "success", "image_path": str(self.image1)},
            {"slot": "A-04", "submit_id": "4", "status": "success", "image_path": str(self.image2)},
        ])
        self.assertNotEqual(proc.returncode, 0)
        self.assertIn("提交位必须且只能是 A-01, A-02", proc.stderr)

    def test_failure_slot_cannot_have_image_path(self):
        proc = self.run_manifest([
            {"slot": "A-01", "submit_id": "1", "status": "success", "image_path": str(self.image1)},
            {"slot": "A-02", "submit_id": "2", "status": "fail", "image_path": str(self.image2)},
        ])
        self.assertNotEqual(proc.returncode, 0)
