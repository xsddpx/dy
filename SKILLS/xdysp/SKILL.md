...
- 每个 `vN` 提交前必须通过主链 03 的生成合同门禁并保留不可变清单；任何终态在宣告完成前必须通过主链 01 的收尾合同门禁。
- Dreamina 成功并下载原始 MP4 后，直接整理为 `OUTPUT/RUN_ID.mp4`；收尾门禁只机械核验文件可读、分辨率和请求时长，不抽取代理帧，不执行人脸、身材、动作、构图或整体观感审查。
