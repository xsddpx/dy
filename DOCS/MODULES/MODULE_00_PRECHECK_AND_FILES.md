...
- 确认本次任务是独立新任务。
- 检查 CDP Chrome、Dreamina CLI、抖音创作者中心登录态、文件读写和固定角色素材；只有显式 `slow` 模式需要检查 Kie API key。
...
- `TEMP/`、`OUTPUT/` 可读写。
- 显式 `slow` 模式额外检查 Kie API key 可用。

## 固定执行账户

`dy auto/fast` 固定使用 macOS 账户 `xsddpx` 的运行环境，不以当前 Codex 所在账户为准；CDP Chrome 固定为 `http://127.0.0.1:9222`，用户目录应为 `/Users/xsddpx/Library/Application Support/Google/Chrome-Codex-CDP`，Dreamina 与抖音创作者中心登录态也以 `xsddpx` 为准。
Kie API key 只在显式 `slow` 模式检查和使用。跨账户执行 Kie 确认图、Dreamina 视频、发布预检或相关自动化命令时必须使用 `sudo -H -u xsddpx ...`，确保 `HOME=/Users/xsddpx`；若 `9222` 已由 `xsddpx` 的 `Chrome-Codex-CDP` 占用且预检通过，直接复用，不关闭、不抢占、不改用当前账户环境。
