...
- 只执行 `anna auto/fast` 单人路线，不接入其他生成模式、角色或兜底工具。
- `fast` 默认在视频生成成功后硬停确认；必须先展示正式视频和关键质检信息，收到用户明确发布授权后才同步发布到抖音和快手。
...
- Dreamina 视频只允许单图输入，固定使用 `anna.png`；参考宫格只用于分析。
...
- 环境修复和最佳实践沉淀只看模块 00；行程、参考和 prompt 规范只看模块 01；视频输入与确认节点只看模块 02；发布只看模块 03；记录收尾只看模块 04。
