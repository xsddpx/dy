...
- `auto` 表示：参考抽帧 -> 提示词 -> Kie Nano Banana Pro 1K 确认图 -> 确认图选择 -> Dreamina 双图视频生成 -> 抖音发布 -> 记录收尾。
- 项目的两个关键生成节点是确认图和视频生成：确认图决定人物、构图和画面锚点是否可用，视频生成决定最终成片动作、节奏和发布质量；用户说“关键节点”时，仅指这两个关键生成节点，不得自行扩展到参考选择、提示词、发布前等其他流程节点。
