...
- 记录本次衣柜编号、衣柜图片路径、服装描述路径、固定环境图路径、三张输入的 SHA-256 和正式视频路径。`xdy` 记录固定角色代理图、首中尾视频代理图，以及胸部体量是否明显偏小的质检结论；`xdysp` 不要求代理图，质检状态记录为 `not_performed` 并注明由用户本人检查。
...
- 每个 `vid-prompt-vN.txt` 记录对应 Dreamina 任务 ID、TNS/终态和下载路径；重试到 `v5` 时保留完整版本链。衣柜编号、三张图片路径和哈希在全部版本中保持不变。
