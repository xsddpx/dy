...
5. 用 `prompt_lint.py derive` 从 `grid-prompt.txt` 派生 fast/slow 阶段 prompt。
...
显式 `slow` 模式才需要 Kie 确认图；prompt 派生统一使用 `TOOLS/prompt_lint.py derive`，细节见模块 02/03。生图前先在本地 `.env` 配置 `KIE_API_KEY`，不要提交 `.env`。遇到环境问题时再按模块 00 修复。
