...
- 本模块只执行阶段裁剪：从 `TEMP/RUN_ID/grid-prompt.txt` 删除完整的 `整体动画：` 段和完整的 `背景音乐：` 段，得到 `TEMP/RUN_ID/A-01-img-prompt-v1.txt`。
...
- `A-01-img-prompt-v1.txt` 只删除动画和音乐两段，其余文本与 `grid-prompt.txt` 保持一致。
